...
\hdashrule[0.5ex]{\textwidth}{1pt}{3mm}
  Actual Result \\
{\footnotesize
This was done 1000 times, extracting the source list and WCS for 1000
...
requirement, using:\\
wcsFlagsPercent =
(np.size(np.where(haswcs\_flags))/np.size(haswcs\_flags))*100.0*u.percent\\
\strut \\
...
To extract the PSF, the following line was executed for each dataId:\\
psf = c.getPsf()\\
...where "c" is a calexp that has been retrieved from the butler.\\
\strut \\
A number of these were displayed at random X, Y positions on the images
to confirm that they are well-formed and retrievable at any arbitrary
position.\\
...
follows:\\
\strut \\
Percentage of fields meeting the threshold: 94.0 \% -\/- False\\
\strut \\
...
\hdashrule[0.5ex]{\textwidth}{1pt}{3mm}
  Actual Result \\
{\footnotesize
Upon examination, we find that many images have only \textasciitilde50
